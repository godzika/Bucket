...
The dashboard lists **one folder at a time** via `GET /api/filesystem`. `GET /api/files` is still a flat list of the current user's files (newest first, `limit`/`offset`) and is not folder-scoped. The UI does not use that flat list.
...
- File create requests accept only a safe basename; path-like filenames are rejected (`422`). Folder uploads create folders first, then create each file in its target folder.
...
- Pending and failed file rows appear in the folder listing (status badge). Download and share require `status=ready`.
...
The UI (`FileBrowser`) can create, rename, and delete folders, and bulk-delete **files** (N× `DELETE /api/files/{id}`; no bulk API). The API `PATCH /api/filesystem/folders/{id}` can also **move** a folder by setting `parent_folder_id`; the current UI does not expose move.
...
The in-tab upload queue preserves directory drops and `<input webkitdirectory>` selections. Uploads land in the folder currently open on the dashboard (`?folder=`). Browser files with an empty `type` are registered as `application/octet-stream`.
...
Auth (public):
- `POST /api/auth/register` — `{ "email", "password" }` (email is lowercased; also creates the user's root folder)
...
Shares (JWT required):
- `POST /api/files/{file_id}/shares` — optional `expires_in_seconds` (60–30 days) and `password` (4–128 chars); `409` if the file is not `ready`
...
Tests are integration tests against the live Postgres + MinIO stack. `tests/conftest.py` rewrites `S3_PUBLIC_ENDPOINT_URL` to `S3_ENDPOINT_URL` for the test process so presigned URLs are reachable from inside the API container.
...
- `tests/test_auth.py` — register / login / me
...
`scripts/smoke_test.py` is an HTTP client against the **already running** API (it does not rewrite the public S3 endpoint). Default compose signs URLs for `http://localhost:5173`, so either:
...
- **Wrong files after switching accounts in the same tab.** React Query keys are not user-scoped (`staleTime` 30s) and the upload queue is a module singleton. Hard-refresh after login, or wait for refetch. Logout does not clear the in-memory queue.
