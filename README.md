...
|---|---|---|
| `JWT_SECRET` | `change-me-...` | **Set this** in production (min 16 chars) |
...
Health checks: `GET /health` on the API, `GET /healthz` on nginx (Docker web).

## Filesystem model

Each user has one root folder named **My files**, created at registration (unique per owner). Files store a `parent_folder_id`; folder placement is tracked in Postgres while object bytes remain in S3 under opaque keys like `users/{user_id}/{file_id}/{filename}`.

The dashboard lists **one folder at a time** via `GET /api/filesystem`. `GET /api/files` is still a flat list of the current user's files (newest first, `limit`/`offset`) and is not folder-scoped. The UI does not use that flat list (`useFilesList` exists but is unused).

`GET /api/filesystem` applies `limit` (default 100, max 200) **separately** to child folders and to files. There is no `offset`. A folder with more than `limit` children of either kind is truncated with no next-page cursor.

Folder rules:
- Names are trimmed, max 255 characters. They cannot be `.` or `..`, and cannot contain `/` or `\`.
- Sibling names are unique **case-insensitively** per parent (`409` on collision).
- File create requests accept only a safe basename (max 255); path-like filenames are rejected (`422`). Folder uploads create folders first, then create each file in its target folder.
- Empty files (`size_bytes: 0`) are allowed by the API. The browser collector skips empty files and common system files (`.DS_Store`, `Thumbs.db`, `desktop.ini`).
- The root folder cannot be renamed, moved, or deleted.
- A folder cannot be moved into itself or into one of its descendants.
- Deleting a folder deletes the subtree. Unlike single-file `DELETE` (metadata commit, then best-effort storage cleanup), folder delete currently aborts/deletes child objects **before** the DB commit.
- File rows use `status` `pending` while uploading and `ready` after `/complete`. Download and share require `ready`. The schema also allows `failed`, but the API never writes that value today.
- Pending rows appear in the folder listing (status badge).
...
The UI (`FileBrowser`) can create, rename, and delete folders, and bulk-delete **files** (N× `DELETE /api/files/{id}`; no bulk API; partial failures are reported). The API `PATCH /api/filesystem/folders/{id}` can also **move** a folder by setting `parent_folder_id`; the current UI does not expose move.

## Upload flow

Files **≤ 5 GB** use a single presigned PUT. Files **> 5 GB** use S3 multipart upload (parts uploaded directly to storage; the API only signs URLs and finalizes). The `Content-Type` header is part of the PUT signature — the client must send the same type it registered. `/complete` HEADs the stored object and persists the **actual** `ContentLength`, not the size declared at create.

The sidebar copy still says “5 GB max per file”. That is the **single-PUT** cap (`SINGLE_PUT_MAX_BYTES`). Multipart uploads may be up to `MAX_FILE_BYTES` (5 TiB), limited also by S3’s 10 000-part ceiling.
...
If a single PUT object is larger than `SINGLE_PUT_MAX_BYTES` at `/complete`, the API deletes the object and the pending row (`413`). The same cleanup runs if the final object exceeds `MAX_FILE_BYTES`.

### Batch and folder uploads

The in-tab upload queue preserves directory drops and `<input webkitdirectory>` selections. Uploads land in the folder currently open on the dashboard (`?folder=`). Browser files with an empty `type` are registered as `application/octet-stream`. Axios metadata calls use a 120s timeout so large folder batches are not aborted early.
...
- A create batch resolves all `ensure-paths` against the **first item’s** parent folder. Wait for the current batch to leave `creating` before dropping files into a different folder.
- The tab must stay open until queued uploads complete. The queue is a module singleton and is not persisted across reloads or cleared on logout.
- Failed uploads best-effort `DELETE` the pending file record; users can retry failed queue items.
- **Cancel/abort does not call `DELETE`.** Cancelled items can leave `pending` rows (and in-flight multipart uploads) until you delete them from the browser.

## Endpoints

Auth (public):
- `POST /api/auth/register` — `{ "email", "password" }` (password 8–128 chars; email is lowercased; `409` if already registered; also creates the user's root folder)
- `POST /api/auth/login` — form fields `username` (= email) + `password`, returns JWT (`email` is lowercased)
...
- `GET /api/files` — flat list of the current user's files (`limit` default 50, max 200, `offset` default 0)
- `GET /api/files/{file_id}` — metadata
- `GET /api/files/{file_id}/download` — presigned GET URL (`409` until ready)
- `DELETE /api/files/{file_id}` — delete metadata + object (or abort in-flight multipart). Cross-user access returns `404`, not `403`.

Filesystem (JWT required):
- `GET /api/filesystem?folder_id={id}` — list one folder's breadcrumbs, child folders, and child files; omit `folder_id` for root (`limit` default 100, max 200, applied separately to folders and files)
...
Shares (JWT required):
- `POST /api/files/{file_id}/shares` — optional `expires_in_seconds` (60–30 days, or omit/`null` for no expiry) and `password` (4–128 chars); `409` if the file is not `ready`
- `GET /api/files/{file_id}/shares` — list shares for a file
- `DELETE /api/files/{file_id}/shares/{token}` — revoke

Public (no auth):
- `GET /api/public/{token}` — share metadata (filename, size, password_protected, expires_at); `410` if expired
- `POST /api/public/{token}/download` — body `{ "password": "..." }` if protected (`401` on wrong password); returns presigned URL

Health:
- `GET /health` — API
- `GET /healthz` — nginx (Docker web)
...
Move a folder (API only; not in the UI):

```bash
curl -s -X PATCH "http://localhost:8000/api/filesystem/folders/$FOLDER_ID" \
  -H "Authorization: Bearer $TOKEN" \
  -H 'Content-Type: application/json' \
  -d "{\"parent_folder_id\":\"$OTHER_FOLDER_ID\",\"name\":\"docs\"}"
```

API-only (no web/Vite): send `Origin: http://localhost:19000` from the **host**, so presigned URLs hit the mapped MinIO port. From inside the `api` container, use `Origin: http://minio:9000` instead (`localhost:19000` is not MinIO there).

## Tests

Tests are integration tests against the live Postgres + MinIO stack. `tests/conftest.py` rewrites `S3_PUBLIC_ENDPOINT_URL` to `S3_ENDPOINT_URL` whenever they differ, so presigned URLs are reachable from inside the API container.
...
Coverage of note:
- `tests/test_files.py` — single PUT, multipart, batch (mixed PUT/multipart, empty items, unsafe filenames), cross-user 404
- `tests/test_folders.py` — root listing, create, `ensure-paths` + batch placement, name collision, cross-user folder 404, delete cascade
- `tests/test_auth.py` — register / login / me, duplicate email, invalid login
...
`scripts/smoke_test.py` is an HTTP client against the **already running** API (it does not rewrite the public S3 endpoint and sends no `Origin`). Default compose signs URLs for `http://localhost:5173`, so either:

- run it from the **host** with web/Vite up (`python3 scripts/smoke_test.py`), or
- recreate the API with an Origin the runner can PUT to, e.g. `S3_PUBLIC_ENDPOINT_URL=http://minio:9000` and `docker compose exec api python scripts/smoke_test.py`

Do not run the smoke script inside the API container against the default compose env — the PUT target will be `localhost:5173`, which is not MinIO from inside that container.

## Troubleshooting

- **`SignatureDoesNotMatch` on upload/download.** The signed host (including port) must match what the browser calls. Use the SPA origin (`http://localhost:5173` or `http://<lan-ip>:5173`). nginx `/files/` forwards `Host` as `$http_host` (port included); `$host` strips the port and breaks SigV4 on `:5173`. (`/api/` uses `$host`; that path is not SigV4.)
...
- **Smoke test PUT fails from `docker compose exec api`.** The script has no `Origin` header, so the running API signs with `S3_PUBLIC_ENDPOINT_URL` (`localhost:5173` by default). That host is not MinIO inside the API container. See Tests above.
...
- **Folder listing looks incomplete.** `GET /api/filesystem` has no pagination. Raise `limit` (max 200) or split the folder; folders and files are capped independently.
- **Cancelled upload still shows in the folder.** Cancel only aborts the browser transfer. Delete the leftover `pending` row from the UI (or `DELETE /api/files/{id}`).
- **Wrong files after switching accounts in the same tab.** React Query keys are not user-scoped (`staleTime` 30s, `refetchOnWindowFocus` off) and the upload queue is a module singleton. Hard-refresh after login, or wait for refetch. Logout does not clear the in-memory queue. A 401 on a protected route does clear JWT (`fileshare.auth` in `localStorage`) and redirects to `/login?next=...` (skipped on `/login`, `/register`, `/s/`).
- **Sidebar says 5 GB max.** That is the single-PUT limit. Multipart (files > 5 GB) is allowed up to `MAX_FILE_BYTES`.
...
- JWT secret must be rotated for production; access tokens are short-lived (60 min default). Stored in `localStorage` as `fileshare.auth`.
- Presigned URLs are time-limited; revoke shares via DELETE.
- CORS is currently `allow_origins=["*"]` with `allow_credentials=False` (see `app/main.py`). Lock origins to the real SPA domain in production.
...
The UI is a single-page React app served by **nginx** in production builds. It uses JWT (stored in `localStorage` as `fileshare.auth`) to talk to the API, browses Drive-like folders, uploads through a persistent **in-tab** queue, and renders a Linear/Notion-inspired layout with dark mode.
...
- `/s/:token` — public share page (no auth; password gate if needed). A stale JWT in `localStorage` is ignored here so guests are not bounced to `/login`.
- Sidebar **Shared** is a placeholder (`/shared` is not routed).
...
- Folder move in the UI; pagination for large folders
- User-scoped React Query keys and upload-queue reset on logout
- Delete pending rows (and abort multipart) when the user cancels an upload
